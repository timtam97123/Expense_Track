...
NOTE: You must already have a master list built by Excel with item amounts in a column (C in this case, an indicator if the receipt exists or not with an indicator "Y" (Column K) for yes "N" for no reciepts. 
You can directly contact me for an excel template as well if interested. 
